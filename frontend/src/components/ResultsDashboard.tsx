    ...
    <div className="flex-grow w-full max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {/* Tab Filter buttons */}
          <div className="flex gap-2 p-1 bg-slate-100 dark:bg-slate-900 border border-slate-200/50 dark:border-slate-800/50 rounded-2xl w-max">
            <button
              onClick={() => setActiveTab('all')}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition ${activeTab === 'all' ? 'bg-white dark:bg-slate-800 shadow-sm text-brand-primary' : 'text-slate-500 dark:text-slate-400'}`}
            >
              {t.allSchemes} ({results.length})
            </button>
            <button
              onClick={() => setActiveTab('eligible')}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition ${activeTab === 'eligible' ? 'bg-white dark:bg-slate-800 shadow-sm text-brand-accent' : 'text-slate-500 dark:text-slate-400'}`}
            >
              {t.eligibleOnly} ({results.filter(r => r.status === 'Eligible').length})
            </button>
            <button
              onClick={() => setActiveTab('partial')}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition ${activeTab === 'partial' ? 'bg-white dark:bg-slate-800 shadow-sm text-brand-warning' : 'text-slate-500 dark:text-slate-400'}`}
            >
